...
def create_interactive_plots(predictions):
    ...
    fig.add_trace(go.Scatter(y=predictions_flat, mode='lines', name='預測值'), row=1, col=1)
...
def main():
    ...
    model_path = st.sidebar.text_input("模型檔案路徑", value="CNN_squat_best.keras")
    scaler_path = st.sidebar.text_input("標準化器檔案路徑", value="scaler_CNN_squat_best.pkl")

    files_exist = all([os.path.exists(model_path), os.path.exists(scaler_path)])
    if not files_exist:
    ...
    with col1:
        st.header("📹 影片上傳與分析")
        uploaded_file = st.file_uploader("選擇要分析的影片檔案", type=['mp4', 'avi', 'mov', 'mkv'])
        ...
        if uploaded_file is not None:
            ...
            if st.button("🚀 開始分析", type="primary"):
                ...
                try:
                    avg_score, detailed_stats, predictions = evaluator.evaluate_video(temp_video_path, progress_callback=update_progress)
                    if avg_score is not None:
                        status_text.text("✅ 分析完成！")
                        st.session_state['analysis_results'] = {'avg_score': avg_score, 'detailed_stats': detailed_stats, 'predictions': predictions, 'video_name': uploaded_file.name}
                    else:
